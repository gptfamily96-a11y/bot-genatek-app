...
• لراحة من الحيرة والتجربة العشوائية بعد فهمك للأسباب الحقيقية`,
  ...
  const selectedMain = mainMenu.find(r => r.id === id);
  if (selectedMain) {
    await sendText(to, selectedMain.title);
    return;
  }

  const selectedPkg = packagesMenu.find(r => r.id === id);
  if (selectedPkg) {
    await sendText(to, selectedPkg.title);
    return;
  }
});
